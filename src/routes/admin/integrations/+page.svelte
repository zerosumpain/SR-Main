...
<script lang="ts">
  ...
  async function saveRename(c: CredRow) {
    ...
    busyAction = 'renaming';
    try {
      const res = await fetch(`/admin/integrations/${c.id}`, {
        method: 'PATCH',
  ...
  async function deleteCredential(c: CredRow) {
    ...
    busyAction = 'deleting';
    try {
      const res = await fetch(`/admin/integrations/${c.id}`, { method: 'DELETE' });
...
<div class="wrap">
  <header class="page-hdr">
    <div>
      ...
      <h1>Third-party credentials</h1>
      <p class="sub">
        Encrypted credentials for external services. Used by workflow nodes.
        Add credentials here, then reference them in canvas nodes.
      </p>
    </div>
