<script lang="ts">
  ...
  const step = $derived(page.url.searchParams.get('step') ?? 'evidence');
  const tabs = [['evidence', 'Policy evidence'], ['actors', 'Actor map'], ['builder', 'Game builder'], ['runner', 'Scenario runner'], ['results', 'Results'], ['audit', 'Audit']];
  ...
  let sensitivityText = $state('[{"assumption_id":"value-3-0","low":0,"high":10,"steps":5}]');
  ...
  async function action(resource: string, body: Record<string, unknown>) {
    ...
    try {
      ...
      success = 'Saved.'; await invalidateAll(); return value;
  ...
  async function addSource() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error); await invalidateAll(); success = 'Source saved.';
...
{#if step === 'evidence'}
  ...
  {#if draft.source}
    ...
    <button disabled={busy} onclick={() => action('extraction-jobs', { task: 'extract-objectives' })}>Propose initial model and objectives</button>
    <button disabled={busy} onclick={() => action('extraction-jobs', { task: 'extract-mechanisms' })}>Review operative mechanisms with model</button>
    ...
    {#if draft.candidate}<ReviewList items={relevantItems} evidence={draft.candidate.evidence} {busy} approve={ids => action('approvals', { item_ids: ids })} edit={editItem} />{/if}
  {/if}
{:else if step === 'actors'}
  <h2>Actors, objectives and information</h2>
  {#if game && draft.candidate}
    <button disabled={busy} onclick={() => action('extraction-jobs', { task: 'propose-actors' })}>Propose actors</button>
    <button disabled={busy} onclick={() => action('extraction-jobs', { task: 'actor-objectives-constraints' })}>Propose objectives and constraints</button>
    <button disabled={busy} onclick={() => action('extraction-jobs', { task: 'information-asymmetries' })}>Review information asymmetries</button>
    ...
    <ReviewList items={relevantItems} evidence={draft.candidate.evidence} {busy} approve={ids => action('approvals', { item_ids: ids })} edit={editItem} />
  {:else}<p>Add a source and candidate model first.</p>{/if}
{:else if step === 'builder'}
  <h2>Strategies, payoffs and item approvals</h2>
  ...
  {#if game}
    <button disabled={busy} onclick={() => action('extraction-jobs', { task: 'propose-strategies' })}>Propose strategies</button>
    <button disabled={busy} onclick={() => action('extraction-jobs', { task: 'causal-relationships' })}>Propose causal relationships</button>
    <h3>Qualitative payoff matrix</h3>
    <div class="table-wrap"><table><thead><tr><th>Strategy profile</th>{#each game.outcome_metrics as metric}<th>{metric.name} ({metric.unit})</th>{/each}</tr></thead><tbody>{#each game.payoff_table as row}<tr><td>{Object.values(row.profile).join(' / ')}</td>{#each game.outcome_metrics as metric}{@const a = game.assumptions.find(a => a.id === row.outcome_values[metric.id])}<td>{a?.value_or_range == null ? 'Unknown' : JSON.stringify(a.value_or_range)}<br />Assumption: {a?.id ?? 'Missing'}</td>{/each}</tr>{/each}</tbody></table></div>
  {/if}
  <details><summary>Add or restructure model / evidence as validated JSON</summary>
  ...
  {#if draft.candidate}<ReviewList items={relevantItems} evidence={draft.candidate.evidence} {busy} approve={ids => action('approvals', { item_ids: ids })} edit={editItem} />{/if}
{:else if step === 'runner'}
  <h2>Model completeness</h2>
  {#if errors.length}<details open><summary>{errors.length} blocking validation issues</summary><ul>{#each errors as issue}<li>{issue}</li>{/each}</ul></details>{:else}<p>All draft items are approved and the payoff table is complete.</p>{/if}
  <button disabled={busy || errors.length > 0} onclick={() => action('versions', {})}>Save immutable approved model version</button>
  ...
  <label>Simulation type <select bind:value={simulationType}><option value="normal-form">Normal-form: pure Nash equilibria</option><option value="sequential">Sequential: backward induction where supported</option><option value="repeated">Repeated simulation</option><option value="agent-based">Agent-based exploration</option></select></label>
  ...
  <label>Random seed <input type="number" min="0" max="4294967295" bind:value={seed} /></label><label>Rounds <input type="number" min="1" max="1000" bind:value={rounds} /></label>
  <label>Parameter overrides: assumption ID → number <textarea rows="5" bind:value={parameterText}></textarea></label>
  <button disabled={busy || !data.versions.length} onclick={() => runModel()}>Run approved scenario</button>
  <h2>Sensitivity: one parameter at a time</h2>
  <label>Ranges (inside approved bounds) <textarea rows="6" bind:value={sensitivityText}></textarea></label>
  <button disabled={busy || !data.versions.length} onclick={() => runModel(true)}>Run sensitivity analysis</button>
