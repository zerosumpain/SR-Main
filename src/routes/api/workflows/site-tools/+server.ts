...
import { offeredOn } from '$lib/workflows/site-tools/registry-internal';
 ...
 * wipe surfaces) are excluded — they can never be selected — and so is a tool
 * whose `surfaces` list leaves out 'workflow'. Auth is enforced by
 * hooks (owner-gated authed area), same as the sibling workflow API routes.
 */
export const GET: RequestHandler = async () => {
  const tools = getTools()
    .filter((t) => !isDenylistedTool(t.name) && offeredOn(t, 'workflow'))
    .map((t) => ({
      name: t.name,
      toolset: t.toolset,
