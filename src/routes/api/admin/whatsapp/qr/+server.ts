 ...
 *
 * Pairing used to happen inside the gateway, so the site never had a screen for
 * it — which is fine right up until the moment the session moves and there is
 * nowhere to scan.
 *
...
export const GET: RequestHandler = async () => {
  const bridgeUrl = !ownsWhatsAppSession() ? (whatsappBridgeUrl(env) ?? '') : '';
  ...
  if (bridgeUrl) {
    ...
    try {
      ...
      if (!res.ok) {
        // A bridge without /qr — fall back to its health so the page can still
        // say whether WhatsApp is up.
