...
/**
 * The shared copy, downloadable.
 *
 * Same document, same module, REDACTED artefacts — `resolveShare` has already
 * run `shareableReport` over them, so the policy paper and the owner's other
 * assessments are gone before this sees the list. The cover names what is
 * withheld rather than letting the file look complete.
 *
...
export const GET: RequestHandler = async (event) => {
  ...
  return assessmentDocument(shared.artefacts, {
    title: shared.title,
