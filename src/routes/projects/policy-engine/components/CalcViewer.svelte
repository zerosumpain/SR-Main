...
<style>
  .cv { border: 1px solid rgba(28,22,17,0.16); border-radius: var(--radius-sharp); background: rgba(255,255,255,0.45); padding: 12px 14px; margin: 12px 0; }
  ...
  .cv-lab { font-family: var(--font-mono); font-size: var(--fs-label-xs); text-transform: uppercase; letter-spacing: 0.08em; color: rgba(28,22,17,0.5); }
  .cv-pick select { font-family: var(--fs-serif); font-size: var(--fs-body-sm); font-weight: 600; color: var(--ink); background: transparent; border: none; border-bottom: 2px solid #b4632e; padding: 1px 2px; cursor: pointer; }
  .cv-yr { font-family: var(--font-mono); font-size: var(--fs-label-xs); color: rgba(28,22,17,0.5); }
  .cv-toggle { display: inline-flex; background: rgba(28,22,17,0.07); padding: 2px; border-radius: var(--radius-sharp); border: 1px solid rgba(28,22,17,0.12); }
  .cv-toggle button { background: transparent; border: none; color: var(--ink); padding: 4px 12px; border-radius: var(--radius-sharp); font-family: var(--font-mono); font-size: var(--fs-label-xs); cursor: pointer; }
  ...
  .t-op { font-family: var(--font-mono); color: rgba(28,22,17,0.4); }
  .t-lab { font-size: var(--fs-label); color: var(--ink); }
  .t-exp { font-style: normal; font-size: var(--fs-label-xs); color: rgba(28,22,17,0.45); margin-left: 4px; }
  .t-sym { font-family: var(--font-mono); font-size: var(--fs-label-xs); color: rgba(28,22,17,0.5); text-align: right; }
  .t-val { font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 600; text-align: right; min-width: 56px; }
  .t-note { grid-column: 1 / -1; margin: 2px 0 0 24px; font-size: var(--fs-label-xs); line-height: 1.4; color: rgba(28,22,17,0.55); }
  .eq-row.result { border-bottom: none; border-top: 1.5px solid rgba(28,22,17,0.3); margin-top: 3px; padding-top: 7px; }
  .eq-row.result .t-lab { font-family: var(--fs-serif); font-weight: 600; font-size: var(--fs-nav); }
  .t-val.big { font-family: var(--fs-serif); font-size: 22px; color: var(--ink); }
  .t-val.big i { font-style: normal; font-size: var(--fs-label-xs); opacity: 0.55; margin-left: 2px; }
  .reconcile { margin-top: 7px; font-family: var(--font-mono); font-size: var(--fs-label-xs); color: var(--success); background: var(--success-bg); border-radius: var(--radius-sharp); padding: 5px 8px; line-height: 1.5; }
  ...
  .part { display: flex; justify-content: space-between; gap: 10px; background: transparent; border: none; padding: 2px 4px; cursor: pointer; border-radius: var(--radius-sharp); }
  .part:hover { background: rgba(28,22,17,0.05); }
  .p-lab { font-size: var(--fs-label-xs); color: rgba(28,22,17,0.75); }
  .p-val { font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 600; }
  .parts-note { margin: 3px 0 0; font-size: var(--fs-label-xs); color: rgba(28,22,17,0.5); }

  .code { font-family: var(--font-mono); font-size: var(--fs-label-xs); line-height: 1.6; white-space: pre-wrap; background: rgba(28,22,17,0.05); border-left: 2px solid var(--accent-ink); border-radius: var(--radius-sharp); padding: 9px 11px; color: rgba(28,22,17,0.85); overflow-x: auto; }
  .code-live { margin-top: 8px; }
  .cl-lab { font-family: var(--font-mono); font-size: var(--fs-label-xs); text-transform: uppercase; letter-spacing: 0.06em; color: rgba(28,22,17,0.5); }
  .cl-vals { display: flex; flex-wrap: wrap; gap: 5px 12px; margin-top: 4px; }
  .cl-vals span { font-family: var(--font-mono); font-size: var(--fs-label-xs); color: rgba(28,22,17,0.7); }
  ...
  .side-lab { font-family: var(--font-mono); font-size: var(--fs-label-xs); text-transform: uppercase; letter-spacing: 0.08em; color: rgba(28,22,17,0.5); }
  .no-lev { font-size: var(--fs-label-xs); color: rgba(28,22,17,0.55); }
</style>
