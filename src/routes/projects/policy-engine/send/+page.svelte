...
<style>
  .block { margin: 34px 0; }
  .pe-h3 { font-family: var(--fs-serif); font-weight: 600; font-size: var(--fs-body); color: var(--ink); margin: 26px 0 8px; }
  .cap { margin: 0 0 16px; font-size: var(--fs-nav); line-height: 1.6; color: rgba(28,22,17,0.72); max-width: 92ch; }
  .note { margin: 12px 0 0; font-size: var(--fs-label); line-height: 1.6; color: rgba(28,22,17,0.76); max-width: 96ch; }
  .inline-link { font-family: var(--font-mono); font-size: var(--fs-label-xs); color: var(--accent-ink); text-decoration: none; border-bottom: 1px dashed currentColor; margin-left: 6px; }

  /* 1 · hero */
  .hero99 { display: flex; align-items: center; gap: 22px; flex-wrap: wrap; padding: 18px 22px; border-radius: var(--radius-sharp);
    border: 1.5px solid var(--error-border); background: var(--error-bg); max-width: 96ch; }
  .h99-big { font-family: var(--fs-serif); font-weight: 600; font-size: clamp(56px, 9vw, 96px); line-height: 0.9; color: #8a2d3a; }
  .h99-lab { margin: 0; flex: 1 1 320px; font-size: var(--fs-body-sm); line-height: 1.55; color: rgba(28,22,17,0.8); }
  .h99-kicker { margin: 12px 0 0; font-size: var(--fs-label); line-height: 1.6; color: rgba(28,22,17,0.72); max-width: 96ch; }

  /* charts */
  .chart-scroll { overflow-x: auto; background: rgba(255,255,255,0.4); border: 1px solid rgba(28,22,17,0.1); border-radius: var(--radius-sharp); padding: 10px; }
  .chart-scroll svg { display: block; width: 100%; min-width: 560px; height: auto; }
  .grid { stroke: rgba(28,22,17,0.1); stroke-width: 1; }
  .ax { font-family: var(--font-mono); font-size: var(--fs-label-xs); fill: rgba(28,22,17,0.55); }
  .area { fill: rgba(138,45,58,0.1); }
  .curve { fill: none; stroke: #8a2d3a; stroke-width: 2.2; }
  .dot { fill: #8a2d3a; }
  .pt-lab { font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 700; fill: #8a2d3a; }
  .tbar { fill: rgba(138,45,58,0.75); }
  .bar-lab { font-family: var(--font-body); font-size: var(--fs-label-xs); fill: rgba(28,22,17,0.75); }
  .bar-lab .b { font-weight: 700; fill: var(--ink); }
  .cbar { fill: rgba(47,111,151,0.55); }
  .cbar.hi { fill: rgba(138,45,58,0.75); }
  .cbar-name { font-family: var(--font-body); font-size: var(--fs-label-xs); font-weight: 600; fill: #fff; }

  /* pipeline funnel */
  .pf-name { font-family: var(--font-body); font-size: var(--fs-label-xs); font-weight: 600; fill: var(--ink); }
  .pf-pct { font-family: var(--font-mono); font-size: var(--fs-label-xs); fill: rgba(28,22,17,0.55); }
  ...
  .pf-val { font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 700; fill: #fff; }
  .pf-val.light { fill: #fff; }
  .pf-div { stroke: rgba(28,22,17,0.2); stroke-width: 1; stroke-dasharray: 3 3; }
  .pf-divlab { font-family: var(--font-mono); font-size: var(--fs-label-xs); fill: rgba(28,22,17,0.5); }
  .pf-detail { margin-top: 14px; display: flex; flex-direction: column; gap: 5px; max-width: 96ch; }
  .pf-line { margin: 0; font-size: var(--fs-label); line-height: 1.55; color: rgba(28,22,17,0.78); }
  .pf-tag { font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 700; color: var(--accent-ink);
    background: var(--accent-ink-tint-12); border-radius: var(--radius-sharp); padding: 1px 6px; margin-right: 5px; white-space: nowrap; }

  /* stat tiles */
  .stat-tiles { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(230px, 100%), 1fr)); gap: 10px; margin-top: 14px; }
  .st { border: 1px solid rgba(28,22,17,0.14); border-radius: var(--radius-sharp); background: rgba(255,255,255,0.45); padding: 12px 14px; }
  .st.warn { border-color: var(--error-border); background: var(--error-bg); }
  .st-big { font-family: var(--fs-serif); font-weight: 600; font-size: clamp(20px, 3vw, 27px); color: var(--ink); }
  .st.warn .st-big { color: #8a2d3a; }
  .st-lab { margin: 5px 0 0; font-size: var(--fs-label-xs); line-height: 1.5; color: rgba(28,22,17,0.72); }
  ...
  .m2 { border: 1px solid rgba(28,22,17,0.16); border-left: 3px solid #8a2d3a; border-radius: var(--radius-sharp); background: rgba(255,255,255,0.45); padding: 13px 15px; display: flex; flex-direction: column; gap: 5px; }
  .m2-big { font-family: var(--fs-serif); font-weight: 600; font-size: clamp(24px, 4vw, 34px); color: #8a2d3a; line-height: 1; }
  .m2-q { font-family: var(--fs-serif); font-style: italic; font-size: var(--fs-label); color: var(--ink); }
  .m2-what { margin: 2px 0 0; font-size: var(--fs-label-xs); line-height: 1.5; color: rgba(28,22,17,0.78); }
  .m2-cited { font-family: var(--font-mono); font-size: var(--fs-label-xs); color: rgba(28,22,17,0.5); margin-top: 2px; }

  /* funding elements */
  .elements { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(220px, 100%), 1fr)); gap: 10px; margin-top: 12px; }
  .el { border: 1px solid rgba(28,22,17,0.14); border-left: 3px solid var(--accent-ink); border-radius: var(--radius-sharp); background: rgba(255,255,255,0.45); padding: 12px 14px; }
  .el.e3 { border-left-color: #8a2d3a; }
  .el-tag { font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: rgba(28,22,17,0.55); }
  .el-amt { display: block; font-family: var(--fs-serif); font-weight: 600; font-size: 22px; color: var(--ink); margin: 3px 0 2px; }
  .el-what { margin: 0; font-size: var(--fs-label-xs); line-height: 1.5; color: rgba(28,22,17,0.74); }
  ...
  .ot::before { content: ''; position: absolute; left: -5px; top: 16px; width: 8px; height: 8px; border-radius: var(--radius-pill); background: #8a2d3a; }
  .ot-date { flex: 0 0 108px; font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 700; color: #8a2d3a; padding-top: 2px; }
  .ot-title { font-family: var(--fs-serif); font-weight: 600; font-size: var(--fs-nav); color: var(--ink); }
  .ot-det { margin: 3px 0 0; font-size: var(--fs-label-xs); line-height: 1.5; color: rgba(28,22,17,0.72); }

  /* measurement gaps */
  .bs-list { display: flex; flex-direction: column; gap: 10px; max-width: 96ch; }
  .bs { display: flex; gap: 13px; align-items: flex-start; border: 1px dashed var(--error-border); border-radius: var(--radius-sharp);
    background: var(--error-bg); padding: 12px 15px; }
  .bs-n { flex-shrink: 0; width: 26px; height: 26px; border-radius: var(--radius-pill); background: #8a2d3a; color: #fff;
  ...
  .bs-gap { font-family: var(--fs-serif); font-weight: 600; font-size: var(--fs-body-sm); color: #8a2d3a; }
  .bs-det { margin: 0; font-size: var(--fs-label); line-height: 1.55; color: rgba(28,22,17,0.76); }

  /* system edges */
  .edges { display: flex; flex-direction: column; gap: 10px; max-width: 96ch; }
  .edge { border: 1px solid rgba(28,22,17,0.14); border-left: 3px solid var(--accent-ink); border-radius: var(--radius-sharp); background: var(--accent-ink-tint-06); padding: 12px 15px; }
  .edge-t { font-family: var(--fs-serif); font-weight: 600; font-size: var(--fs-body-sm); color: var(--ink); }
  .edge-d { margin: 4px 0 0; font-size: var(--fs-label); line-height: 1.55; color: rgba(28,22,17,0.78); }

  /* isos panel */
  .isos { border: 1px solid rgba(28,22,17,0.16); border-left: 3px solid #b4632e; border-radius: var(--radius-sharp); background: rgba(180,99,46,0.05); padding: 14px 16px; max-width: 96ch; }
  .isos-head { display: flex; align-items: center; justify-content: space-between; gap: 10px; flex-wrap: wrap; }
  .isos-t { font-family: var(--fs-serif); font-weight: 600; font-size: var(--fs-body-sm); color: var(--ink); }
  .isos-body { margin: 9px 0 4px; font-size: var(--fs-label); line-height: 1.6; color: rgba(28,22,17,0.8); }
  .isos-props { margin: 6px 0 4px; padding-left: 20px; display: flex; flex-direction: column; gap: 3px; }
  .isos-props li { font-size: var(--fs-label-xs); line-height: 1.5; color: rgba(28,22,17,0.76); }

  /* asks */
  .asks { display: flex; flex-direction: column; gap: 9px; max-width: 96ch; margin-bottom: 14px; }
  .ask { display: flex; gap: 12px; align-items: flex-start; border: 1px solid var(--success-border); border-radius: var(--radius-sharp); padding: 11px 14px; background: var(--success-bg); }
  .ask-n { flex-shrink: 0; width: 26px; height: 26px; border-radius: var(--radius-pill); background: var(--success); color: #fff;
    font-family: var(--font-mono); font-size: var(--fs-label); display: inline-flex; align-items: center; justify-content: center; }
  .ask-body { display: flex; flex-direction: column; gap: 3px; }
  .ask-t { font-family: var(--fs-serif); font-weight: 600; font-size: var(--fs-nav); color: var(--ink); }
  .ask-w { margin: 0; font-size: var(--fs-label-xs); line-height: 1.5; color: rgba(28,22,17,0.74); }
  .closer { border: 1px solid var(--accent-ink-tint-35); border-left: 3px solid var(--accent-ink); border-radius: var(--radius-sharp);
    background: var(--accent-ink-tint-06); padding: 13px 16px; max-width: 96ch; }
  .closer p { margin: 0; font-size: var(--fs-label); line-height: 1.6; color: rgba(28,22,17,0.78); }
