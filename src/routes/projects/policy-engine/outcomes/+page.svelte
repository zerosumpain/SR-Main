<script lang="ts">
  ...
  interface Theme { key: string; title: string; prose: string; charts: ChartDef[]; }
...
<div class="pe-route wide">
  ...
  {#each themes as th (th.key)}
    <section class="theme">
      ...
      <p class="theme-prose">{th.prose}</p>
      <div class="grid">
        {#each th.charts as c (c.title)}
          ...
          <div class="cell">
            ...
            <p class="summary tone-{sm.tone}">{sm.text}</p>
          </div>
        {/each}
      </div>
    </section>
...
<style>
  ...
  .theme { margin: 26px 0; }
  .theme-prose { margin: 0 0 14px; font-size: 12.5px; line-height: 1.55; color: rgba(28,22,17,0.66); max-width: 86ch; }
