...
<style>
  ...
  .ex { border: 1px solid rgba(28,22,17,0.13); border-top: 3px solid var(--tc); border-radius: var(--radius-sharp); padding: 12px 14px;
  ...
  .ex-place { font-family: var(--fs-serif); font-weight: 600; font-size: var(--fs-body-sm); color: var(--ink); }
  .ex-who { font-family: var(--font-mono); font-size: var(--fs-label-xs); color: rgba(28,22,17,0.5); }
  .ex-tier { flex-shrink: 0; font-family: var(--font-mono); font-size: var(--fs-label-xs); text-transform: uppercase; letter-spacing: 0.05em;
    color: #fff; padding: 3px 7px; border-radius: var(--radius-sharp); white-space: nowrap; }
  .ex-what { margin: 0; font-size: var(--fs-label-xs); line-height: 1.5; color: rgba(28,22,17,0.74); }
  .ex-result { margin: 0; font-size: var(--fs-label-xs); line-height: 1.5; color: rgba(28,22,17,0.78); }
  .ex-result b { color: var(--ink); }
  .ex-lesson { margin: 0; font-size: var(--fs-label-xs); line-height: 1.5; color: var(--ink); }
  .ex-tag { display: block; font-family: var(--font-mono); font-size: var(--fs-label-xs); text-transform: uppercase; letter-spacing: 0.08em; color: var(--tc); margin-bottom: 2px; }
  .ex-caveat { margin: 0; font-size: var(--fs-label-xs); line-height: 1.45; color: #8a2d3a; }
  .ex-src { margin-top: auto; align-self: flex-start; font-family: var(--font-mono); font-size: var(--fs-label-xs); color: var(--accent-ink); text-decoration: none; border-bottom: 1px dashed currentColor; }

  /* 3 · London effect decomposition */
  .le { background: rgba(255,255,255,0.4); border: 1px solid rgba(28,22,17,0.1); border-radius: var(--radius-sharp); padding: 14px 16px; max-width: 88ch; }
  .le-lab { display: block; font-family: var(--font-mono); font-size: var(--fs-label-xs); text-transform: uppercase; letter-spacing: 0.08em; color: rgba(28,22,17,0.55); margin-bottom: 9px; }
  .le-bar { display: flex; height: 38px; border-radius: var(--radius-sharp); overflow: hidden; }
  .le-seg { display: flex; align-items: center; justify-content: center; min-width: 0; }
  .le-pct { font-family: var(--fs-serif); font-weight: 600; font-size: var(--fs-body); color: #fff; }
  .le-key { display: flex; gap: 16px; flex-wrap: wrap; margin-top: 9px; }
  .le-k { display: inline-flex; align-items: center; gap: 6px; font-size: var(--fs-label-xs); color: rgba(28,22,17,0.7); }
  .le-k i { width: 11px; height: 11px; border-radius: var(--radius-sharp); display: inline-block; flex-shrink: 0; }
  .le-note { margin: 10px 0 0; font-size: var(--fs-label-xs); line-height: 1.55; color: rgba(28,22,17,0.65); }

  /* 4 · programme gantt */
  .gantt-scroll { overflow-x: auto; }
  .gantt { position: relative; min-width: 720px; padding: 26px 0 8px; }
  .g-year { position: absolute; top: 0; transform: translateX(-50%); font-family: var(--font-mono); font-size: var(--fs-label-xs); color: rgba(28,22,17,0.45); }
  ...
  .g-nowlab { position: absolute; top: 6px; transform: translateX(-50%); font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 600; color: #9a3b2e; }
  .g-row { position: relative; display: grid; grid-template-columns: 215px 1fr; gap: 4px 10px; align-items: center; margin: 7px 0; }
  .g-name { font-size: var(--fs-label-xs); line-height: 1.3; color: var(--ink); font-weight: 500; }
  .g-name small { display: block; font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 400; color: rgba(28,22,17,0.5); }
  .g-track { position: relative; height: 16px; }
  .g-bar { position: absolute; top: 0; height: 16px; border-radius: var(--radius-sharp); opacity: 0.85; }
  .g-bar.open { border-radius: var(--radius-sharp) 0 0 4px; -webkit-mask-image: linear-gradient(90deg, #000 75%, transparent); mask-image: linear-gradient(90deg, #000 75%, transparent); }
  ...
  .g-status { grid-column: 1 / -1; font-size: var(--fs-label-xs); line-height: 1.45; color: rgba(28,22,17,0.6); margin: 0 0 4px; }
  .g-key { display: flex; gap: 14px; flex-wrap: wrap; margin-top: 12px; }
  .g-kk { display: inline-flex; align-items: center; gap: 5px; font-family: var(--font-mono); font-size: var(--fs-label-xs); color: rgba(28,22,17,0.6); }
  ...
  .own { border: 1px solid rgba(28,22,17,0.12); border-left: 3px solid var(--oc); border-radius: var(--radius-sharp); padding: 10px 12px;
    background: rgba(255,255,255,0.4); display: flex; flex-direction: column; gap: 4px; }
  .own-name { font-family: var(--fs-serif); font-weight: 600; font-size: var(--fs-nav); color: var(--oc); }
  .own-holds { font-size: var(--fs-label-xs); line-height: 1.45; color: rgba(28,22,17,0.74); }
  .own-gap { font-size: var(--fs-label-xs); line-height: 1.4; font-weight: 600; color: #8a2d3a; }
