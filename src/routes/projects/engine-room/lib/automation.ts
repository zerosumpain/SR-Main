...
export const FANIN_STORY = {
  ...
  incident: 'A daily spending summary did exactly this, twice: one branch wiped out the other, and three further branches collapsed into one. The transform downstream found nothing where it expected data, emitted empty strings rather than failing, and the run died three nodes later with an unexplained HTTP error. Nothing in the build, the save, or the visual editor had flagged it.',
