...
function buildChunks(): Chunk[] {
  ...
  for (const s of STAGES)
    add({ id: `stage-${s.id}`, sourceKey: 'trace-stages', sourceType: 'trace', title: `Stage ${s.no} of a turn: ${s.name}`, url: `${B}/trace`,
      text: `${s.blurb} In plain terms: ${s.eli5Blurb}` });
  for (const l of LAYERS)
    add({ id: `layer-${l.id}`, sourceKey: 'trace-layers', sourceType: 'trace', title: `Layer ${l.no} of the stack: ${l.name} (${l.tag})`, url: `${B}/trace`,
  ...
  for (const s of STAGES)
    for (const l of LAYERS) {
      ...
      add({ id: `cell-${s.id}-${l.id}`, sourceKey: `trace-${s.id}`, sourceType: 'trace',
        title: `${s.name} × ${l.name}: ${c.label}`, url: `${B}/trace`,
        text: `${c.detail} In plain terms: ${c.eli5}${c.hazard ? ` KNOWN FAILURE MODE: ${c.hazard}` : ''}` });
    }
  for (const sc of SCENARIOS)
    add({ id: `scenario-${sc.id}`, sourceKey: 'trace-scenarios', sourceType: 'trace', title: `Traced example: ${sc.label}`, url: `${B}/trace`,
      text: `${sc.prompt} ${sc.note} Stage by stage: ${sc.stages.map((x, i) => `${STAGES[i].name} — ${x.say}`).join(' ')}` });

  // ---- models ----
  for (const r of RESOLUTION)
    add({ id: `res-${r.n}`, sourceKey: 'model-resolution', sourceType: 'models', title: `Model resolution layer ${r.n}: ${r.name}`, url: `${B}/models`,
      text: `${r.what} When it applies: ${r.when}` });
  add({ id: 'sellers', sourceKey: 'sellers', sourceType: 'models', title: 'Sellers: one model id, many providers', url: `${B}/models`,
  ...
  add({ id: 'ttft-seller', sourceKey: 'sellers', sourceType: 'models', title: 'Latency is seller variance, not prompt length', url: `${B}/models`,
  ...
  add({ id: 'reasoning-floor', sourceKey: 'reasoning', sourceType: 'models', title: 'The reasoning-token floor and the empty answer', url: `${B}/models`,
  ...
  add({ id: 'cache-story', sourceKey: 'caching', sourceType: 'models', title: 'Prompt caching — the largest single cost lever', url: `${B}/models`,
  ...
  add({ id: 'auction', sourceKey: 'auction', sourceType: 'models', title: 'The nightly model auction', url: `${B}/models`,
  ...
  // ---- conversation ----
  add({ id: 'hermes', sourceKey: 'engine', sourceType: 'chat', title: 'An external agent runtime behind the chat', url: `${B}/chat`,
  ...
  add({ id: 'segments', sourceKey: 'frames', sourceType: 'chat', title: 'One reply, many message segments', url: `${B}/chat`,
  ...
  add({ id: 'accidental-invariant', sourceKey: 'frames', sourceType: 'chat', title: 'The accidental invariant', url: `${B}/chat`,
  ...
  add({ id: 'watchdog', sourceKey: 'watchdog', sourceType: 'chat', title: 'Timeouts chosen by what the turn is doing', url: `${B}/chat`,
  ...
  add({ id: 'stream-consts', sourceKey: 'stream', sourceType: 'chat', title: 'Streaming constants', url: `${B}/chat`,
    text: STREAM_CONSTANTS.map((c) => `${c.k}: ${c.v} — ${c.why}`).join(' ') });
  add({ id: 'resume', sourceKey: 'stream', sourceType: 'chat', title: 'Resume, never replay', url: `${B}/chat`,
  ...
  add({ id: 'presence', sourceKey: 'stream', sourceType: 'chat', title: 'A signal that is wrong exactly when needed', url: `${B}/chat`,
  ...
  for (const m of MCP_FACTS)
    add({ id: `mcp-${m.k.slice(0, 12)}`, sourceKey: 'mcp', sourceType: 'tools', title: `Protocol server — ${m.k}: ${m.v}`, url: `${B}/tools`, text: m.why });

  // ---- memory ----
  for (const s of SIGNALS)
    add({ id: `sig-${s.id}`, sourceKey: 'entity-resolution', sourceType: 'memory', title: `Matching signal: ${s.label}${s.base ? ` (${s.base})` : s.delta ? ` (+${s.delta})` : ''}`, url: `${B}/memory`,
      text: `${s.what} ${s.why}` });
  add({ id: 'er-overview', sourceKey: 'entity-resolution', sourceType: 'memory', title: 'How entity resolution decides', url: `${B}/memory`,
  ...
  add({ id: 'retrieval', sourceKey: 'retrieval', sourceType: 'memory', title: 'Retrieval and embeddings', url: `${B}/memory`,
    text: RETRIEVAL.map((r) => `${r.k}: ${r.v} — ${r.why}`).join(' ') });
  add({ id: 'datastore', sourceKey: 'datastore', sourceType: 'memory', title: 'The flexible datastore', url: `${B}/memory`,
  ...
  for (const g of GRAPH_FACTS)
    add({ id: `graph-${g.title.slice(0, 14)}`, sourceKey: 'graph', sourceType: 'memory', title: g.title, url: `${B}/memory`,
      text: `${g.body} Lesson: ${g.lesson}` });

  // ---- research ----
  add({ id: 'fabrication', sourceKey: 'fabrication', sourceType: 'research', title: 'Where fabrication comes from', url: `${B}/research`,
  ...
  for (const c of CATEGORIES)
    add({ id: `cat-${c.id}`, sourceKey: 'node-categories', sourceType: 'automation', title: `Node category: ${c.name}`, url: `${B}/automation`, text: c.what });
  add({ id: 'engine', sourceKey: 'engine-facts', sourceType: 'automation', title: 'The workflow engine', url: `${B}/automation`,
    text: `${NODE_COUNT} node types across six categories. ${ENGINE_FACTS.map((f) => `${f.k} — ${f.v}: ${f.why}`).join(' ')}` });
  add({ id: 'fanin', sourceKey: 'fan-in', sourceType: 'automation', title: 'The fan-in collision', url: `${B}/automation`,
  ...
  add({ id: 'cron-utc', sourceKey: 'cron', sourceType: 'automation', title: 'Scheduled jobs run in UTC', url: `${B}/automation`,
  ...
  add({ id: 'doctor', sourceKey: 'doctor', sourceType: 'automation', title: 'The nightly workflow doctor', url: `${B}/automation`,
    text: DOCTOR.map((d) => `${d.k}: ${d.why}`).join(' ') });

  // ---- building ----
  for (const p of PHASES)
    add({ id: `phase-${p.id}`, sourceKey: 'nightly-phases', sourceType: 'building', title: `Nightly phase: ${p.name}`, url: `${B}/building`,
      text: `${p.what}${p.note ? ` ${p.note}` : ''} In plain terms: ${p.eli5}` });
  add({ id: 'gate', sourceKey: 'verify-gate', sourceType: 'building', title: 'The verification gate on generated tools', url: `${B}/building`,
  ...
  for (const c of CANDIDATES)
    add({ id: `cand-${c.id}`, sourceKey: 'verify-examples', sourceType: 'building', title: `Gate example: ${c.label}`, url: `${B}/building`,
      text: `${c.intent} ${c.verdict}` });
  add({ id: 'backlog', sourceKey: 'nightly', sourceType: 'building', title: 'Memory between nights', url: `${B}/building`,
  ...
  add({ id: 'builder', sourceKey: 'builder', sourceType: 'building', title: 'The autonomous builder', url: `${B}/building`,
  ...
  for (const p of PIPELINE)
    add({ id: `pipe-${p.id}`, sourceKey: 'pipeline', sourceType: 'shipping', title: `Pipeline stage: ${p.name} (${p.where})`, url: `${B}/shipping`,
      text: `${p.what} Why it is shaped this way: ${p.why} In plain terms: ${p.eli5}` });
  for (const s of SAFETY)
    add({ id: `safe-${s.title.slice(0, 16)}`, sourceKey: 'deploy-safety', sourceType: 'shipping', title: s.title, url: `${B}/shipping`, text: s.body });
  add({ id: 'risk-tier', sourceKey: 'risk', sourceType: 'shipping', title: 'What a machine may merge by itself', url: `${B}/shipping`,
