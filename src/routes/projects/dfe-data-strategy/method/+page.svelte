...
<style>
  ...
  .formula { background: rgba(28,22,17,0.05); border: 1px solid rgba(28,22,17,0.1); border-radius: var(--radius-sharp); padding: 12px 14px; font-family: var(--font-mono); font-size: var(--fs-label-xs); line-height: 1.6; color: rgba(28,22,17,0.8); overflow-x: auto; white-space: pre; }
  .pe-prose { max-width: 80ch; }
  .pe-prose code { background: rgba(28,22,17,0.06); padding: 1px 4px; border-radius: var(--radius-sharp); font-family: var(--font-mono); font-size: max(0.9em, var(--fs-label-xs)); }
  .coeffs { display: grid; gap: 8px; }
  .coeff { display: grid; grid-template-columns: 160px 48px 1fr; gap: 10px; align-items: baseline; }
  .coeff code { font-family: var(--font-mono); font-size: var(--fs-label-xs); color: #8a2d3a; }
  .cf-v { font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 600; color: var(--ink); }
  .cf-n { font-size: var(--fs-label-xs); line-height: 1.45; color: rgba(28,22,17,0.66); }
  .caveats { max-width: 80ch; padding-left: 18px; }
  .caveats li { font-size: var(--fs-label); line-height: 1.55; color: rgba(28,22,17,0.74); margin-bottom: 6px; }
  .ftable { display: flex; flex-direction: column; gap: 4px; max-width: 86ch; }
  .frow { display: grid; grid-template-columns: 40px 1fr auto; gap: 12px; align-items: baseline; padding: 5px 10px; border: 1px solid rgba(28,22,17,0.1); border-radius: var(--radius-sharp); background: rgba(255,255,255,0.4); }
  .frow.out { opacity: 0.6; }
  .f-n { font-family: var(--fs-serif); font-size: var(--fs-body-sm); font-weight: 600; color: var(--accent-ink); text-align: right; }
  .f-f { font-size: var(--fs-label); color: var(--ink); }
  .f-v { font-family: var(--font-mono); font-size: var(--fs-label-xs); color: rgba(28,22,17,0.55); max-width: 300px; text-align: right; }
  ...
  .gloss dt { font-family: var(--font-mono); font-size: var(--fs-label-xs); font-weight: 600; color: var(--accent-ink); }
  .gloss dd { margin: 2px 0 0; font-size: var(--fs-label-xs); line-height: 1.5; color: rgba(28,22,17,0.7); }
  .sources { columns: 2; column-gap: 28px; padding-left: 18px; }
  .sources li { font-size: var(--fs-label-xs); line-height: 1.4; color: rgba(28,22,17,0.7); margin-bottom: 4px; break-inside: avoid; }
