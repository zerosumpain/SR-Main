// relationships.ts — how the department's data relationships actually play out across the sector:
...
export interface Relationship {
  ...
  name: string;
  who: string;
  /** Data the department takes from this partner. */
  flowsIn: RelFlow[];
  /** What the department gives back. */
  flowsOut: RelFlow[];
...
export const RELATIONSHIPS: Relationship[] = [
  {
    ...
    flowsIn: [
      ...
      { what: 'Daily attendance data (via Wonde)', detail: 'Session-level attendance extracted from each school’s MIS and passed to the department every day via Wonde; mandatory since the 2024/25 academic year after a voluntary trial.' },
      ...
      { what: 'Financial returns', detail: 'Academies accounts returns and maintained-school consistent financial reporting feed the department’s financial benchmarking service.' },
    ],
    flowsOut: [
      { what: 'Analyse School Performance / View your education data', detail: 'The department’s sign-in service dashboards returning a school’s own attainment and context data for self-evaluation and inspection preparation.' },
      ...
      { what: 'Financial Benchmarking and Insights Tool (FBIT)', detail: 'Compares spending against similar schools and trusts; the department reports 87% of schools and academies engaged with it in its first year.' },
    ],
    mandate:
  ...
  },
  {
    ...
    mandate:
      'Children’s social care returns (CIN census, SSDA903) are statutory under s.83 Children Act 1989; SEN2, the early years census and the preference collections are mandatory departmental collections (the Childcare Act 2006 underpins early years). The Children’s Wellbeing and Schools Act 2026 adds Children Not in School registers — a new LA data duty.',
    friction:
      'Capacity: ADCS argues new granular collections should be fully funded and warns LA data capability varies widely; the department has funded work on reducing children’s-social-care data burdens even as SEN2 went person-level and the 2026 Act adds registers.',
  ...
  },
  {
    id: 'ofsted',
    name: 'Ofsted',
    who: 'The inspectorate for schools, early years, FE and children’s services — a non-ministerial department reporting to Parliament, not to the department.',
    flowsIn: [
      { what: 'Inspection outcomes', detail: 'Inspection judgements and monthly management information feed the department’s statistics and intervention decisions, and link from every school’s GIAS record.' },
      { what: 'LA children’s-services (ILACS) outcomes', detail: 'Ofsted’s judgements on council children’s services inform the department’s improvement interventions in local authorities.' },
    ],
    flowsOut: [
      { what: 'Inspection Data Summary Report (IDSR) inputs', detail: 'The department’s census, attainment and attendance data is packaged into Ofsted’s IDSR, used by inspectors to prepare for and guide every school inspection.' },
      { what: 'GIAS registry data', detail: 'Ofsted takes school opens/closes, type, phase, religious character and predecessor links from the department’s Get Information about Schools database.' },
    ],
    mandate:
      'Ofsted inspects under the Education Act 2005 and was constituted by the Education and Inspections Act 2006, reporting to Parliament. Data moves between the two under sharing agreements: The department owns the collections, Ofsted owns the judgement.',
    friction:
      'The independence dynamic: Ofsted insists data “does not drive judgements” while schools experience the IDSR as the department data with enforcement teeth; after single headline grades were scrapped from Sept 2024, GIAS removed its Ofsted rating fields — a visible loosening of the data coupling.',
    wants: 'Timely pupil-level the department data for risk assessment and inspection preparation — without being cast as the department’s enforcement arm.',
    sources: [
  ...
  },
  {
    ...
    who: 'The qualifications regulator (Ofqual), the department’s Standards and Testing Agency for primary assessment, and the exam awarding organisations.',
    flowsIn: [
      { what: 'GCSE/A level results from awarding organisations', detail: 'Awarding organisations supply results covering the vast majority of pupils direct to the department for KS4/KS5 performance measures, amended through autumn checking exercises.' },
      ...
      { what: 'Phonics and EYFSP results via LAs', detail: 'Phonics screening and early years foundation stage profile outcomes reach the department through local authority returns.' },
    ],
    flowsOut: [
      { what: 'GRADE research dataset', detail: 'A joint Ofqual–the department–UCAS initiative linking micro-data from all three (2017–2022) for independent research via the ONS secure research environment.' },
      { what: 'Validation feedback to awarding organisations', detail: 'The department checks supplied results (e.g. qualification-number validity) and reports discrepancies back to awarding organisations for review.' },
    ],
    mandate:
    ...
    friction:
      'Boundary-holding: the data estate is deliberately split — Ofqual holds exam-level data, the department holds the pupil spine — so system-wide questions need negotiated linkage (GRADE took a dedicated joint initiative); the 2025 handover of test operations to Pearson moved schools onto a new portal mid-cycle.',
    wants: 'Linked pupil-level context for standard-setting and research — while preserving regulatory independence from the department.',
    sources: [
  ...
  },
  {
    ...
    flowsIn: [
      { what: 'Individualised learner record (ILR)', detail: 'Learner-level returns from colleges, training providers and apprenticeship employers up to 14 times a year — collected directly by the department since ESFA closed on 31 March 2025.' },
      { what: 'HESA student record (Data Futures)', detail: 'Jisc, which absorbed HESA, collects the annual student record from HE providers to a specification set by the OfS; the department is a statutory customer of the data.' },
      { what: 'UCAS admissions data', detail: 'UCAS application and acceptance data is linked into the department’s LEO dataset and the joint GRADE research dataset.' },
    ],
    flowsOut: [
      ...
      { what: 'Learning Records Service', detail: 'The department operates the LRS, issuing unique learner numbers and holding personal learning records that providers and awarding bodies query.' },
    ],
    mandate:
      'ILR returns are a condition of the department funding agreements rather than a census statute; the student record is statutory under the Higher Education and Research Act 2017, with Jisc designated data body for England (from Oct 2022) collecting to OfS specification.',
    friction:
      'The Data Futures re-platforming of the HESA record was troubled enough that the OfS commissioned an independent review after the strained 2022/23 collection; post-ESFA, FE providers deal directly with the department for ILR compliance and funding-reconciliation deadlines.',
  ...
  },
  {
    ...
    flowsIn: [
      ...
      { what: 'Health advice into EHC needs assessments', detail: 'ICBs provide health advice and provision detail in EHC needs assessments, which surfaces in LAs’ person-level SEN2 returns to the department.' },
    ],
    flowsOut: [
  ...
  },
  {
    ...
    who: 'The Whitehall departments whose data the department links to — and which come to the department for pupil matching.',
    flowsIn: [
      ...
      { what: 'DWP/HMRC/Home Office checks for FSM eligibility', detail: 'The department’s Eligibility Checking System queries DWP, HMRC and Home Office data so LAs can verify free school meals and early-years entitlements; a rebuilt ECS rolls out from 1 June 2026.' },
    ],
    flowsOut: [
      { what: 'Pupil matching for the Home Office', detail: 'Under a December 2015 MoU, the department matched pupils’ school and home address details against Home Office requests — capped at 1,500 children a month — for immigration enforcement.' },
      { what: 'Transparency-logged external shares', detail: 'NPD-level shares with the Home Office and police have been listed in the department’s “external data shares” transparency publication since December 2017.' },
    ],
    mandate:
    ...
    friction:
      'The Home Office deal began in secret in 2015, was exposed via FOI in 2016, fuelled the Boycott School Census campaign, and forced the department to scrap nationality/country-of-birth collection (2018) and delete the data (2020) — a lasting trust scar over the whole NPD.',
    wants: 'Reliable identity and eligibility matching against the department’s pupil spine — enforcement and entitlement checking, not education insight.',
    sources: [
  ...
  },
  {
    ...
    flowsIn: [
      { what: 'Research accreditation regime (DEA 2017/UKSA)', detail: 'The department relies on UKSA research accreditation and ONS-run secure environments to vet every researcher and project that touches NPD or LEO extracts.' },
      { what: 'ADR UK investment', detail: 'ADR UK (ESRC-funded) pays for curation of department-derived flagship research datasets — LEO, ECHILD and GRADE.' },
    ...
    flowsOut: [
      { what: 'NPD/LEO deposits into the ONS Secure Research Service', detail: 'De-identified NPD and LEO extracts sit in the ONS SRS as the department’s default researcher-access route, gatekept by the department’s Data Sharing Approval Panel (DSAP).' },
      { what: 'Flagship linked datasets', detail: 'The department data became national research infrastructure: LEO and ECHILD are ADR UK flagship datasets, and GRADE links the department, Ofqual and UCAS micro-data.' },
    ],
    mandate:
    ...
    friction:
      'Platform churn: ONS’s Integrated Data Service was meant to replace the SRS but stopped taking new research applications in July 2025, with the SRS continuing “indefinitely” — The department’s research-access route is hostage to the centre’s infrastructure wobbles.',
    wants: 'The department’s linked datasets wired into cross-government infrastructure as exemplar assets for the National Data Library.',
    sources: [
  ...
  },
  {
    ...
    flowsIn: [
      { what: 'Daily attendance extraction via Wonde', detail: 'Wonde, a private intermediary, extracts session attendance from school MIS and transfers it to the department daily at no charge to schools — a commercial pipe inside a statutory flow.' },
      { what: 'Census files generated by MIS', detail: 'MIS software assembles and validates schools’ statutory census and workforce returns before submission to the department — supplier data quality shapes national statistics.' },
    ],
    flowsOut: [
      { what: 'Data standards and framework design', detail: 'The department publishes education data standards and is designing the MIS framework around portability, so data “flows smoothly across school, trust, LA and national levels”.' },
    ...
    mandate:
      'No statute binds suppliers directly: they act as processors inside schools’ and the department’s statutory flows (the attendance collection has a published the department DPIA). From Sept 2027 the relationship shifts from pure market to department-brokered comply-or-explain procurement via the framework.',
    friction:
      'Lock-in politics: The department officials say some suppliers hold “schools’ data as hostage” with exit tactics; the ~£200m market has flipped — SIMS from 74% share (2021) to ~34%, behind Arbor on 39%, Bromcom on 16% — amid litigation such as Bromcom’s High Court claim over a trust-wide MIS deal.',
