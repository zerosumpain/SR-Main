<script lang="ts">
  ...
  import { FieldHeartSimulation } from '$lib/heart-sim/field-sim';
  ...
  let { bpm = 70, particleCount = 12000, class: className = '', onstats }: Props = $props();

  let canvas: HTMLCanvasElement;
  let sim: FieldHeartSimulation | null = null;
  ...
  function build(count: number) {
    ...
    error = null;
    try {
      sim = new FieldHeartSimulation({ canvas, bpm, particleCount: count });
      sim.resize();
      sim.start();
