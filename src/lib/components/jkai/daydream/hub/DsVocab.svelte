<script lang="ts">
  // The daydream CSS VOCABULARY (`.ds-vocab`): band, card, pill, tag, table,
  // button. Moved out of the /jkai/daydreams layout on 2026-09-25 so /home's
  // pages (the household room among them) can wear it without living under
  // that layout. Declared as `:global` rules under a wrapper class rather than
  // imported from a `.css` file, because a standalone CSS import from a route
  // file breaks the vite-pwa build while svelte-check passes clean.
  // `DrillPanel` puts `ds-vocab` on its own portalled panel so the vocabulary
  // reaches it outside this wrapper — which works only while some page that
  // mounts this component is open, as it always is where a drill can open.
  //
  ...
  // unscoped and still requires it, so nothing leaks past a wrapper.
