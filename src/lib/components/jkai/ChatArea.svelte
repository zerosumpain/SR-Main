      ...
      <div class="msg-stack">
        {#each messages as msg, msgIndex (msg.id)}
          ...
          {:else if msg.isProgress}
            ...
            {/if}
            {#if msg.content}
              <!-- Token events populate the progress bubble before the final
                   event changes its lifecycle state. Render that accumulated
                   content now so the reply is genuinely visible as it streams. -->
