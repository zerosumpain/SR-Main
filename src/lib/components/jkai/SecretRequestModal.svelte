<script lang="ts">
  ...
  const missing = $derived(
    request.fields.filter((f) => f.required && !String(values[f.key] ?? '').trim()).map((f) => f.label),
  );
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch('/api/admin/apis/secrets', {
        ...
        body: JSON.stringify({ provider: request.provider, value }),
      });
      if (!res.ok) {
      ...
      onDone({ stored: true, handle: request.destination.handle });
    } catch (e) {
...
<div class="sr-backdrop" use:portal role="presentation">
  ...
  <div
    ...
    role="dialog"
    aria-modal="true"
    aria-label="{request.title} — credential needed"
  >
    <header class="sr-hdr">
      <span class="sr-title">{request.title}</span>
      <span class="sr-badge">credential needed</span>
    </header>
    ...
    <dl class="sr-dest">
      <dt>Stored in</dt>
      <dd>
        <code>{request.destination.store}</code> → <code>{request.destination.handle}</code>
        <span class="sr-note">encrypted at rest</span>
      </dd>
      <dt>Binding</dt>
      <dd>
        {#if request.destination.storeOnly}
          <span class="sr-note">store-only — never attached to any outbound request</span>
        {:else}
          {request.destination.hosts.join(', ') || '—'}
          <span class="sr-note">{request.destination.methods.join('/')} only</span>
        {/if}
      </dd>
      {#each request.companions as c (c.handle)}
        <dt>Also creates</dt>
        <dd>
