<script lang="ts">
  ...
  const preview = (() => {
    ...
    return mode === 'schema' ? 'JSON Schema' : mode;
  })();
...
>
  ...
  <div class="px-3 py-2">
    ...
    <div class="flex flex-col items-end gap-2 mt-2">
      <div class="flex items-center gap-1.5">
        <span class="text-[10px] font-medium" style="color: #2d7d46;">Pass</span>
        <span class="w-2 h-2 rounded-full" style="background: #2d7d46;"></span>
      </div>
      <div class="flex items-center gap-1.5">
        <span class="text-[10px] font-medium" style="color: #b43232;">Fail</span>
        <span class="w-2 h-2 rounded-full" style="background: #b43232;"></span>
      </div>
    </div>
  </div>

  <!-- Pass output handle (right, upper) -->
  <Handle
    type="source"
  ...
  />
  <!-- Fail output handle (right, lower) -->
  <Handle
    type="source"
    position={Position.Right}
    id="fail"
