...
describe('readable HTML text', () => {
	it('preserves boundaries between adjacent block elements', async () => {
		const text = await readableTextFromHtml(
	...
	it('keeps list items readable and removes executable or invisible content', async () => {
		const text = await readableTextFromHtml(
	...
	it('normalises copied whitespace without destroying paragraphs', async () => {
