...
export function answerContract(request: string): AnswerContract {
  ...
  return { depth: detailed ? 'detailed' : 'brief', requestedItems: count ? Number(count[1]) : undefined, needsReview: detailed || /\b(next|live|current|latest|available|balance|integration|api|departure|arrival|forecast)\b/i.test(request) };
}
