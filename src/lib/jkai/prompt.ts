...
- EVERY LINK YOU RENDER MUST RESOLVE, AND MUST BE RELATIVE. Never write a root-absolute internal link like \`href="/chapter-2/"\`. Nobody views this at the server root: the preview is served under a proxy prefix and the published version under \`/projects/<slug>/\`, and a leading \`/\` escapes both — it resolves to the site root and 404s. Write \`../chapter-2/\` and \`./styles.css\` instead. Watch too for template placeholders escaping into the HTML — a literal \`\${...}\` in an href is a 404.
