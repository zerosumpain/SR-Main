 ...
 * kind's size limit, write it to the media store, insert the row, mirror it to
 * /drive.
 *
 * Lifted out of `POST /api/jkai/attachments` so the phone's lane
 * (`/api/native/chat/attachments`) takes exactly the same file through exactly
 * the same checks. Two copies of the sniff-and-limit logic is how one of them
 * ends up accepting what the other refuses.
 *
 * Rejections are thrown as SvelteKit `error(...)`s, which the web route lets
 * through as before and the native route turns back into a JSON body.
 */
