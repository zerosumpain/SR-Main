...
describe('piThinkingLevel', () => {
  ...
  it('clamps the rung the pinned pi predates', () => {
    // `max` exists for Astra and the 5.6 line and reaches Codex through the
    // site's own bridge. A build goes through pi instead, whose catalogue stops
    // at gpt-5.5, so the deepest each provider knows is what it gets.
