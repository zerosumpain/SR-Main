...
export function legendFor(
...
): Array<{ kind: 'node' | 'edge'; color: string; dash?: string; label: string; hint: string }> {
  const out: Array<{ kind: 'node' | 'edge'; color: string; dash?: string; label: string; hint: string }> = [];
  for (const p of ['known', 'new', 'thread'] as const) {
    if (!nodes.some((n) => n.provenance === p)) continue;
    const s = PROVENANCE_STYLE[p];
    out.push({ kind: 'node', color: s.color, label: s.legend, hint: s.hint });
  }
  for (const [key, present] of [
    ...
    out.push({ kind: 'edge', color: s.color, dash: s.dash, label: s.legend, hint: s.hint });
  }
  return out;
}
