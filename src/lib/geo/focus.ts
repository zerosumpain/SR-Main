...
export interface FocusResult {
  ...
  /**
   * DISTINCT cells that changed hands whose centre lies inside `bounds` — the
   * same rule in every branch, including the quiet and no-dominant-cluster
   * fallbacks, which return the home box and so usually count 0.
   *
   * This is deliberately NOT the window's total: that number is already in the
   * payload as `handovers.cells`, and a stat tile sitting under this map must
   * describe the ground the reader can actually see. Matches `MapFocus` in
   * `src/routes/projects/landgrab/types.ts` ("inside the focus bounds").
   */
  changedCells: number;
  label: string;
}
...
/** `bounds` is [[south, west], [north, east]], so this is one comparison per edge. */
function within(bounds: FocusResult['bounds'], ll: LatLon): boolean {
  return (
    ll.lat >= bounds[0][0] && ll.lat <= bounds[1][0] && ll.lon >= bounds[0][1] && ll.lon <= bounds[1][1]
  );
}
...
export function chooseFocus(input: FocusInput): FocusResult {
  ...
  /** Keyed, so a cell listed as both changed and active is one cell. */
  ...
  const add = (t: Tile, w: number, changed: boolean) => {
    ...
    if (changed) changedTiles.set(`${t.x}:${t.y}`, t);
  };
  for (const t of input.changed) add(t, FOCUS.changedWeight, true);
  for (const t of input.active) add(t, FOCUS.activeWeight, false);

  /**
   * The one definition of `changedCells`, applied to whatever box we return.
   * Counting the chosen CLUSTER's cells instead would both miss changed cells
   * in sub-floor bins that the padded bbox happens to cover, and count nothing
   * at all on the two branches that return the home box.
   */
  const changedInside = (bounds: FocusResult['bounds']) => {
    let n = 0;
    for (const t of changedTiles.values()) if (within(bounds, tileCentre(t.x, t.y, zoom))) n += 1;
    return n;
  };
  ...
  if (total === 0) {
    return {
      bounds: homeBounds,
      reason: 'quiet',
      changedCells: changedInside(homeBounds),
      label: `${home.name} · nothing has changed hands`,
    };
  }
  ...
  for (const start of eligible) {
    ...
    const cluster: Cluster = { bins: new Set(), weight: 0, cells: [] };
    const stack = [start];
    seen.add(start);
    while (stack.length) {
      const b = stack.pop()!;
      ...
      for (const t of members.get(b) ?? []) cluster.cells.push(t);
  ...
  const touchesHome = (c: Cluster) => c.cells.some((t) => within(homeBounds, tileCentre(t.x, t.y, zoom)));
  ...
  if (!chosen) {
    // Everything is away but nothing dominates: the home box, counting only
    // what changed hands INSIDE it. The window's total belongs to
    // `handovers.cells`; printing it over an empty Darlington would be a lie.
    return {
      bounds: homeBounds,
      reason: 'home',
      changedCells: changedInside(homeBounds),
