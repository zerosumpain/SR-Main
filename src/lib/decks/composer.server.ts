...
function systemPrompt(ctx: ComposeContext): string {
  return [
    ...
    '- Atmosphere (effect blocks) is seasoning, not sauce: at most ONE background effect on a slide that earns it, and only where the metaphor fits — heartbeat behind living data, starfield behind a vast number, drift behind a quiet statement, plexus behind networks/federation, currents behind movement and flows, orbits behind cycles and the long view, sea behind calm scale, halftone/letterpress behind print-and-language material. Transition wipes (melt, shatter, inkbleed, slats, dissolve) only on chapter openers — never on consecutive slides; melt and shatter are the boldest, at most once or twice per deck.',
