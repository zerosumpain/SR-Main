...
function systemPrompt(ctx: ComposeContext): string {
  return [
    ...
    '- Prose styles are presets — choose deliberately: a short rhythmic creed ("Refusal. Auditability. Blast radius.") → style "band" (inverted emphasis band, add an *italic* second line); three-or-more parallel points of detail → style "cards" (each paragraph a card opened by a **bold title**) instead of a wall of text; sources/footnotes → style "aside".',
