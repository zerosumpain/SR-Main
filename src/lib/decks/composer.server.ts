...
function systemPrompt(ctx: ComposeContext): string {
  return [
    ...
    '- Atmosphere (effect blocks) is seasoning, not sauce: at most ONE background effect on a slide that earns it — heartbeat behind living data, starfield behind a vast number, drift behind a quiet statement. Transition effects only on chapter openers.',
