...
export const openaiTools: OpenAIFunctionDef[] = [
  ...
  zodToFunction('set_trigger', setTriggerSchema, 'Set the workflow trigger type. Use "manual" for user-initiated runs (this is the default; you can omit this tool call for manual workflows), "webhook" for HTTP-triggered (optionally set config.secret to a shared secret; callers must then send it as the X-Webhook-Secret header or the POST is rejected 401), "cron" for scheduled (provide config.expression as a cron string like "0 9 * * *"), or "event" for event-driven.'),
