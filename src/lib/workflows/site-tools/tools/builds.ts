...
register({
  ...
  handler: async (args, toolCtx) => {
    ...
    // lost. Absent whenever the chat is on a stamped default, which leaves every
    // unpinned thread on the `builder` role.
    const ctx = toolCtx?.modelContext ?? (await resolveBuilderModel());
