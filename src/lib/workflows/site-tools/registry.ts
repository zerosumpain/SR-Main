...
export function getToolsetManifest(): Array<{
  ...
  const toolsetDescriptions: Record<string, string> = {
    ...
    visualise: 'Inline visual responses — render charts (Vega-Lite), maps (Leaflet), diagrams (Mermaid), and tables directly in the chat, plus place-name geocoding (OpenStreetMap) so map points land where they are meant to',
