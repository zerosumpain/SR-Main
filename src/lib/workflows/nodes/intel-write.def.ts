...
export const intelWriteDef: NodeDefinition = {
  ...
  basicConfig: [
    ...
    { key: 'sourceTag', label: 'Source tag', type: 'template-textarea', placeholder: 'e.g. cdo-daily-briefing', description: 'Stored in metadata for filtering later.' },
    { key: 'sourceUrl', label: 'Source URL', type: 'template-textarea', placeholder: '{{input.url}}', description: 'Canonical URL for provenance.' },
    { key: 'extraMetadata', label: 'Extra metadata (JSON)', type: 'template-textarea', placeholder: '{ "persona": "cdo-dfe", "score": {{input.score}} }', description: 'Optional JSON merged into the note metadata.' },
  ],
  llmDescription:
