...
export const interactiveStepDef: NodeDefinition = {
  ...
  llmDescription: 'Pauses the workflow for a human to solve a CAPTCHA, log in, or confirm data. Mode="vnc" opens a live headed Chromium in a browser modal pointing at url. Mode="confirm" shows a form with fields. Mode="both" shows both. For CAPTCHA/login setup: use mode="vnc", match the `profile` to the downstream `stealth-scrape` node\'s profile, and set the url to the target site\'s search/landing URL. For data confirmation checkpoints: use mode="confirm" with the appropriate fields. The node outputs {completed, completedAt, formValues, durationMs} — downstream nodes read it as {{nodes.<this-node-label-slug>.formValues.myField}}. Place this node UPSTREAM of stealth-scrape when the target domain has bot-detection or login requirements — the human seeds the profile once and headless runs reuse the cookies.',
  llmExamples: [
    {
      mode: 'vnc',
