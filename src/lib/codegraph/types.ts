// The graph payload contract — what src/lib/codegraph/network.ts produces and
// the NetworkGraph components render.
//
// It lived in src/lib/components/intel/types.ts, which put a domain module's
// own output shape inside the UI tree: codegraph had to import $lib/components
// to describe its own return value. The renderers depend on the producer's
// shape, not the other way round, so the contract belongs here.
//
