...
export function proposalTransport(): ProposalTransport | null {
  ...
  return { async complete(system, data) {
    ...
    const response = await client.chat.completions.create({ model, messages: [{ role: 'system', content: system }, { role: 'user', content: data }], response_format: { type: 'json_object' }, max_tokens: 16000 }, { timeout: 90000, maxRetries: 0 });
