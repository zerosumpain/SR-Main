 ...
 */
export function fitToBudget(artefacts: Artefact[], build: (a: Artefact[]) => unknown, limit: number): Fitted {
  const notes: string[] = [];
  ...
  const order = [...working].sort((a, b) => rank(a) - rank(b));
...
function rank(a: Artefact): number {
