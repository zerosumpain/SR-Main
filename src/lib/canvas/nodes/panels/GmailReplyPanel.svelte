<script lang="ts" module>
  ...
  // in GmailSendPanel — both panels can share the in-memory result during
  // a single session.
  type GmailAccount = {
    id: number;
    email: string;
    ...
    if (!accountsPromise) {
      accountsPromise = fetch('/api/gmail/accounts')
        ...
        .catch((err) => {
          ...
          console.warn('[GmailReplyPanel] failed to load /api/gmail/accounts', err);
          return [];
        });
    }
...
<div class="gr">
  ...
  <section class="gr-sec">
    ...
    {#if !accountsLoaded}
      <p class="gr-empty">Loading accounts…</p>
    {:else if accountsError || accounts.length === 0}
      <label class="gr-field">
        <span class="gr-label">Account ID</span>
        <input
          type="number"
          min="0"
          step="1"
    ...
    {:else}
      <label class="gr-field">
        <span class="gr-label">Account</span>
        <select
