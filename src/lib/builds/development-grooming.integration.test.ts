...
it.skipIf(!local)('persists proposals, retains the original ask, blocks unanswered acceptance and loses no edits on failure or races', async () => {
 ...
 expect((await post({ action: 'brief', questions: 'Which metrics?' })).status).toBe(400);
 ...
 await mutateDelivery(id, 'brief_accepted', s => ({ ...s, brief: { ...s.brief, acceptedAt: new Date().toISOString() } }), row.revision);
 expect((await post({})).status).toBe(400);
