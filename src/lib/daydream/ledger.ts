...
/**
 * The household, for the Family tab: `loadHousehold()` (who is where now, how
 * fresh each track is, the shape of each person's day so far — now in
 * $lib/home/presence/household) plus daydream's per-person detail.
 */
export async function loadFamily() {
  const { FAMILY_SUBJECTS } = await import('./types');
  const { loadHousehold } = await import('$lib/home/presence/household');
