...
// carries: what it said, what it held for you, what it learned, what it would
// like to build — deterministic, every note linked to itself on the feed.
//
...
/** `YYYY-MM-DD` of the local day `daysAgo` before `now` (Europe/London). */
function localDay(now: Date, daysAgo: number): string {
...
export async function buildDaydreamBriefing(now = new Date()): Promise<DaydreamBriefing> {
  ...
  if (themes.length) {
    fact('Learned', themes.map((t) => trim(t.title, 60)).join(' · '), null);
  ...
  if (status === 'empty') lines.push('• A quiet day — nothing said, nothing held.');
