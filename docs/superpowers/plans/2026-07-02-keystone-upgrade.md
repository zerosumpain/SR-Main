...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans (inline, this session) — isolated content tasks may be dispatched to subagents. Steps use checkbox (`- [x]`) syntax for tracking.
...
- [x] Steps: write integrity test → types → org registry (~20 orgs w/ radial layout) → skeleton datasets → tests pass → commit.
...
- [x] Steps: failing tests → implement each module minimal → pass → commit.
...
- [x] Steps: components → nav regroup (labels: Landscape, Commitments, Influence map, Frameworks, Legislation, DfE in context, Sector voices | Author, Policy builder, ◆ Workbench, ◆ Upload | Intel radar | How it works) → briefing rebuild → visual check on dev server → commit.
...
- [x] Steps: merge script/manual synthesis → verification pass (subagent, spot-check ~15 riskiest against live URLs) → tests → commit.
...
- [x] Steps: synthesize → tests (comparator integrity: unique ids, urls) → commit.
...
- [x] Steps: store → page shell + filters + stat band → shelf → timeline → flow map → demand → drawer → cross-link (landscape footer, briefing) → visual check → commit.
...
- [x] Steps: corpus chunks → glossary + Term → method page sections → commit.
...
- [x] Steps: store → page shell → rail → editor (test typing/paste manually via dev server) → guidance panel → autosave verify (reload persists) → commit. **Done + browser-verified (typing, autosave, insert-starter).**
...
- [x] Steps: endpoint (validate function unit-testable in `lib/author/reviewValidate.ts` + test) → VerifyPanel deterministic parts → matrix + gap list → deep review wiring → manual SSE test via curl → commit.
...
- [x] Steps: plan.ts + tests → PlanPanel UI → commit.
...
- [x] Steps: serializeDoc → panel → manual export tests (md, docx, json round-trip) → commit.
...
- [x] Steps: per page (takeaway copy written fresh per page from its data) → visual check → commit.
...
- [x] Steps: finalize scores → build stragglers → docs → commit.

### Task F1: QA + deploy + verify

- [x] `npx vitest run src/routes/projects/dfe-data-strategy`.
- [x] `NODE_OPTIONS=--max-old-space-size=8192 npm run check`.
- [x] Full `npx vitest run`.
- [x] Playwright/browser pass on dev server: briefing, commitments (all 4 lenses), author (all 4 tabs), nav mobile menu.
