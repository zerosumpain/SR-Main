...
| Does an LLM verdict auto-merge? | yes; no | **No** — it moves the score, and `same` at high confidence can reach the existing auto-merge threshold, but the threshold and the chain guard are unchanged | Auto-merging on a model's say-so is the one irreversible-feeling action here, and `autoMergeDuplicates` already has a blast-radius cap that exists for exactly this. | Yes — every merge is in the ledger. |
