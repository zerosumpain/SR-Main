...
| 11 | Which wake raises a notification | background refresh only / HealthKit background delivery only / both | **both** | `BGAppRefreshTask` runs when iOS chooses; `HKObserverQuery` + `enableBackgroundDelivery(.hourly)` was ALREADY wired for uploads on an entitlement the app has always held, is scheduled, and fires exactly when the watched figures change | Yes |
| 12 | Fixing 104 fixed-size font call sites | convert each site to a new ramp / teach the existing helpers `relativeTo:` | **teach the helpers** | 104 sites is not a conversion you finish, and leaving the fixed API in the file means the next view reaches for it. `check-dynamic-type.mjs` on the 1x runner is the gate | Yes |
| 13 | Three palette tones measure 2.58–3.53:1 against a 4.5:1 floor | repaint the palette / state the rule and move text off them | **state the rule** | The palette is the site's, copied by hex and asserted by tests; repainting it is a decision about the site. 47 ghost-on-cream sites moved to `--text-muted` (5.34:1), `--warn` gained a glyph, and `ContrastTests` pins the measurements | Yes — the numbers are now in the repo |
| 14 | A notification with every channel switched off | discard it / write it silently | **write it** | A ledger that records only what was DELIVERED cannot answer "did anything happen while that was off", and switching a category back on would lose the history | Yes |
| 15 | A caller's `minIntervalSeconds` vs the category's floor | caller wins / category wins / the larger wins | **the larger** | The shim passes 60s to collapse a burst; that must not quietly undo a three-hour setting made on the phone | Yes |
| 16 | Answering plan/confirm/approval gates from the phone | build it / defer | **defer** | `PATCH …/chat` already takes the acks and is on the device lane, so it is reachable — but an unattended run should not add an untestable write path that approves commands. The app drops the SSE stream instead, which un-suppresses the WhatsApp escalation that had been silently skipped | n/a — noted as the next change |
| 17 | Renaming the app from "SR Companion" | rename to `sr.` / leave | **leave** | The app outgrew the name, but a Home Screen label changing overnight is a surprise nobody asked for | n/a |
