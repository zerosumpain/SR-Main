...
| D11 | `isSurfacePublic` naming `/health/segments` in the release log | add an exclusion / leave it | **leave it** | that filter blocks only `/admin` and `/api`, and already passes `/jkai`, `/drive` and `/live` — all owner-only. A route NAME is not a GPS trace, and a one-off exclusion here would be inconsistent with the whole rest of the list | yes |
| D12 | The mock series and the four invented correlations | leave as-is / flag / delete | **flag, and withhold the invented ones from anonymous** | both were indistinguishable from measurements on a PUBLIC page, with nothing in the returned type to say otherwise. Deleting the mock would blank the page on a cold start, which is what it is for; deleting the correlations is a bigger call than this brief | yes |
| D13 | Public/owner payload construction | spread `shared` into both / pick from an allow-list | **pick** | spreading means a field added to `shared` next month is public by accident. `PUBLIC_FIELDS` makes each addition a reviewable line, and `disclosureLeaks` is the second belt | yes |
| D14 | Two rank algorithms in the segments code | leave / unify on `rankEfforts` | **unify** | `getActivitySegments` broke ties by sort order while the leaderboard used competition ranking, so the same effort printed "2nd of 11" on one page and "1st of 11" on another. It now also reports the RANKED denominator, because HR-derived metrics null out far more often than `effortCount` suggests | yes |
| D15 | Which ACWR the coach reads (four disagree) | unify them / pick one and name it | **pick `load.trimpAcwr` and display which** | `getReadiness` uses a different one; unifying silently changes the readiness score, which changes the proposed sport and the suggested distance. Unification is its own piece of work | yes |
| D16 | "On the board" leading 32% of rows | accept / add honest families | **add trailing-window, calendar-month, day and vs-typical families** | measured against production: 366 of 1,136 rows led with the bare fallback, which is not "something excellent". None now do; `week_cadence` is the weakest thing that leads, and only where nothing better is true | yes |
