...
| Engine cache share | (a) `cacheRead / inputTokens`, as on the site card; (b) `cacheRead / (cacheRead + inputTokens)` | **(b)** | The two stores count differently and only measurement settles it: 163 of 269 sessions in a 30-day sample record more cache reads than input tokens, so Hermes counts cache reads *beside* input while OpenRouter's `prompt_tokens` already *includes* `cached_tokens`. Sharing one formula printed "433% cached". | Yes |
| Unmeasured cache | (a) treat a null `cache_read_tokens` as zero; (b) measure only over rows that carry a figure | **(b)** | The column was added on 2026-08-22, so every earlier row is null. (a) reported a month of missing instrumentation as "prompt caching is doing almost nothing (0.0%)" — a fabricated performance problem, and the exact class of bug this page exists to catch. | Yes |
