...
- `jkai_extended({operation: 'list'|'schema'|'invoke', name?, args?})` — discover and invoke any of ~125 domain tools (gmail, health, blog, research, scraper, build, ha, render, etc.). Use `list` with `compact: true` for a catalogue survey.
...
- **health (jkai-health)** — sleep, training load, readiness, heart rate, vitals data from Apple Health, Whoop and the bike sensor. Triggers: "how was my sleep last night", "what's my training load", "am I recovered". Tools: `health_sleep`, `health_readiness`, `health_training_load`, `health_stats`, `health_timeline`.
