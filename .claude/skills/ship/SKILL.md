...
> outage plus a public `/admin` exposure via the LAN auth bypass (2026-07-24).
> (Mechanism and diagnosis order are in the `vps-ops` skill, which stays local.) The
...
7. **Verify live.** A green deploy job is not verification.
   - Commit landed:
     `ssh sr-vps cat /opt/strange-rambling-svelte/build/.deploy-sha`
