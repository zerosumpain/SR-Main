...
- **Codex reads images and PDFs, not audio or video** (since 2026-09-25: the
  ...
  content parts must check `getModelCapabilities()` first.
- **A chat attachment is re-sent as a FILE on every later turn**, newest first
  within `MEDIA_BUDGET_BYTES` (`$lib/jkai/media/multimodal`), so a follow-up about
  a photo is answered from the photo. Only what overflows falls back to its
  description. Each upload is mirrored to /drive and stamped with
  `metadata.driveFileId`/`drivePath`; a thread's files move to `jkai/<title>/`
  when it is titled.
...
**Google Cloud Console setup:** The OAuth client needs `https://strangeramblings.com/api/gmail/callback` and `http://localhost:5173/api/gmail/callback` in its authorized redirect URIs, plus the `gmail.modify`, `gmail.send`, `gmail.labels` and (for members) `gmail.readonly` scopes on the consent screen. The app stays **unverified** on purpose — verifying restricted Gmail scopes means an annual paid CASA assessment; members see Google's unverified-app warning once.

**Member mailboxes (intel spaces PR B).** A family member promoted at `/admin/access` can connect their OWN Gmail, read-only (`gmail_accounts.principal_id = 'u_…'`). Only the nightly intel sweep reads it, into their own space. Every owner-facing path — the watcher, workflow nodes, chat's gmail tools, canvas pickers, the connectors probe, `/admin/connections/gmail` — filters to `principal_id = 'owner'` through `$lib/workflows/gmail/owner-accounts`. Any new code that picks a Gmail account must use it too: an unfiltered "most recently updated active account" default resolves to a member's mailbox the moment their token refreshes.
