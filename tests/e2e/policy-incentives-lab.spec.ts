...
test('private synthetic workflow: source, approvals, immutable run, reproducibility and reports', async ({ page, request }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Actors, objectives and information' })).toBeVisible();
...
test('the synthetic workflow can be completed through the interface', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Propose initial model and objectives' })).toBeEnabled();
  await page.getByRole('button', { name: 'Propose initial model and objectives' }).click();
  await expect(page.getByText('ev-scheme · Section 1', { exact: false }).first()).toBeVisible();
  await page.getByRole('link', { name: 'Game builder', exact: true }).click();
  ...
  await page.getByRole('button', { name: 'Save immutable approved model version' }).click();
