...
/**
 * A finished turn's `analyse` link is ONE feature served by TWO fields, and the
 * chat route has TWO branches. Three of those four combinations were right.
 *
 * `ChatArea` reads `result.traceId` while the turn is finishing and
 * `metadata.traceId` after a reload. `handleWithLoop` set only the second, so
 * once chat moved off Hermes the button vanished the moment a toolchain
 * completed — and came back if you reloaded the page, which is the shape that
 * makes it look like a rendering bug rather than a missing field.
 *
 ...
 * has to catch the NEXT branch that forgets. A behavioural test through the
 * route would pin the two call sites that exist today and say nothing about a
 * third.
 */
...
describe('a completed toolchain always offers its trace', () => {
  ...
  it.each(['handleWithHermes', 'handleWithLoop'])(
