...
describe('extractArticle', () => {
  it('extracts title and body text from article HTML', async () => {
    ...
    const out = await extractArticle(html, 'https://example.com/a');
    expect(out.text).toMatch(/first paragraph/);
    expect(out.title).toMatch(/Hello|Test/);
  });

  it('returns empty text when input is junk', async () => {
    const out = await extractArticle('<html></html>', 'https://a.com');
    expect(out.text).toBe('');
  });

  it('does not throw on invalid HTML', async () => {
    const out = await extractArticle('<<not-html', 'https://a.com');
